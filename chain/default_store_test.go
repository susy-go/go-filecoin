...
func newChainStore(dstP *DefaultSyncerTestParams) chain.Store {
	r := repo.NewInMemoryRepo()
	ds := r.Datastore()
	return chain.NewDefaultStore(ds, hamt.NewCborStore(), dstP.genCid)
}
...
func TestGetByKey(t *testing.T) {
	...
	gotG := requireGetTsas(ctx, t, chain, dstP.genTS.ToSortedCidSet())
	got1 := requireGetTsas(ctx, t, chain, dstP.link1.ToSortedCidSet())
	got2 := requireGetTsas(ctx, t, chain, dstP.link2.ToSortedCidSet())
	got3 := requireGetTsas(ctx, t, chain, dstP.link3.ToSortedCidSet())
	got4 := requireGetTsas(ctx, t, chain, dstP.link4.ToSortedCidSet())

	assert.Equal(t, dstP.genTS, gotG.TipSet)
	assert.Equal(t, dstP.link1, got1.TipSet)
	assert.Equal(t, dstP.link2, got2.TipSet)
	assert.Equal(t, dstP.link3, got3.TipSet)
	assert.Equal(t, dstP.link4, got4.TipSet)

	assert.Equal(t, dstP.genStateRoot, gotG.TipSetStateRoot)
	assert.Equal(t, dstP.link1State, got1.TipSetStateRoot)
	assert.Equal(t, dstP.link2State, got2.TipSetStateRoot)
	assert.Equal(t, dstP.link3State, got3.TipSetStateRoot)
	assert.Equal(t, dstP.link4State, got4.TipSetStateRoot)
}
...
func TestLoadAndReboot(t *testing.T) {
	...
	r := repo.NewInMemoryRepo()
	ds := r.Datastore()
	chainStore := chain.NewDefaultStore(ds, hamt.NewCborStore(), dstP.genCid)
	requirePutTestChain(t, chainStore, dstP)
	assertSetHead(t, chainStore, dstP.genTS) // set the dstP.genesis block

	assertSetHead(t, chainStore, dstP.link4)
	chainStore.Stop()

	// rebuild chain with same datastore
	rebootChain := chain.NewDefaultStore(ds, hamt.NewCborStore(), dstP.genCid)
	err := rebootChain.Load(ctx)
	assert.NoError(t, err)

	// Check that chain store has index
	// Get a tipset and state by key
	got2 := requireGetTsas(ctx, t, rebootChain, dstP.link2.ToSortedCidSet())
	assert.Equal(t, dstP.link2, got2.TipSet)
